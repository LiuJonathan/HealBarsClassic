...
function HealComm:GetNextHealAmount(guid, bitFlag, time, ignoreGUID, casterGUIDfilter)
	local healTime, healAmount, healFrom
	local currentTime = GetTime()

	for _, tbl in pairs({pendingHeals, pendingHots}) do
		for casterGUID, spells in pairs(tbl) do
			if ( not ignoreGUID or ignoreGUID ~= casterGUID ) and (not casterGUIDfilter or casterGUIDfilter == casterGUID) then
				for _, pending in pairs(spells) do
